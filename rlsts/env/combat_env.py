...
class CombatEnv(gym.Env):
    ...
    def __init__(self, config: Optional[dict] = None) -> None:
        ...
        self.observation_space = Dict({
            ...
            "action_mask": Box(low=0, high=1, shape=(self.game.MAX_ACTION,), dtype=np.float32),
        })
        self.action_space = Discrete(self.game.MAX_ACTION)
    ...
    def to_env_obs(self, obs: CombatObservation) -> dict:
        return {
            "character_hp": np.array([obs.character_hp / obs.character_max_hp], np.float32),
